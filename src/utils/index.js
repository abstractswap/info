...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      ...
      (remove ? `remove` : `add`) +
      `/v2/${token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address}/${'ETH'}?chain=ink`
    )
  } else {
    return (
      `https://swap.reservoir.tools/#/` +
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://swap.reservoir.tools/#/swap?chain=ink?inputCurrency=${token0Address}`
  } else {
    return `https://swap.reservoir.tools/#/swap?chain=ink?inputCurrency=${
